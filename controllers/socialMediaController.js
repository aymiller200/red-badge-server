...
router.get('/get-social-media/:GuestId', validate, validateTwo, async (req, res) => {
    try {
...
router.put('/edit/:GuestId/:id', validate, async (req, res) => {
    try {
        if (req.params.GuestId == req.guest.id) {
            const social = await SocialMedia.update(req.body, {
                where: { id: req.params.id }
            })
            res.status(200).json({ message: 'Successfully Updated', social })
        } else {
            res.status(403).json({
